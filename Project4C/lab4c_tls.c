...
void print_cur_report() //mode 1 prints temp, mode 0 doesn't
{
    ...
    if(g_report && (ts.tv_sec >= g_next_time))
    {
        ...
        {char sslbuf[CHUNK];
        sprintf(sslbuf, "%02d:%02d:%02d %0.1f\n", tm->tm_hour, tm->tm_min, tm->tm_sec, rawToTemp(g_rawtemp));
        SSL_write(ssl_client, sslbuf, strlen(sslbuf));}
...
while(1)
{
    ...
    if(ret >= 1)
    {
        int bytes = SSL_read(ssl_client, ssl_pollbuf, CHUNK);
        if(bytes>0)
            ssl_pollbuf[bytes] = '\0';
        else
        {
            fprintf(stderr, "Unable to read from sll server...\n");
            exit(2);
        }
        ...
        if(write == NULL)
        {
            fprintf(stderr, "can't create 'buf9928.txt' for writing...\n%s\n", strerror(errno));
            exit(1);
        }
        fputs(ssl_pollbuf, write);
        fclose(write);
        ...
        while(fgets(pollbuf, CHUNK, read))
